...
- **名前 → UUID 解決は AI 側の責務**: d6e の MCP ツール
  `d6e_execute_workflow` / `d6e_instant_run_stf` はワークフロー名・STF 名を
  直接は受け付けません。UUID が必須です。本アプリのリソースはインストール時に
  `d6e/invoice-jp/` プレフィックスが付くため、AI エージェントは
  `d6e_list_workflows` / `d6e_list_stfs` で UUID を引いてから呼び出す必要が
  あります。この手順は `template_prompt` に明記済みですが、MCP クライアントが
  異常な自動リトライループに入った場合は Slack 等のチャット側タイムアウトで
  失敗することがあります。
